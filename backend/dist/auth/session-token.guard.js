...
let SessionTokenGuard = class SessionTokenGuard {
    ...
    async canActivate(context) {
        ...
        const token = authHeader?.startsWith('Bearer ')
            ...
            : null;
        if (!token) {
            throw new common_1.UnauthorizedException('Session token required. Login first, then send Authorization: Bearer <session_token>.');
        }
        const user = await this.authService.validateSessionToken(token);
        if (!user) {
            throw new common_1.UnauthorizedException('Invalid or expired session token. Please login again.');
        }
        request.user = user;
        return true;
    }
