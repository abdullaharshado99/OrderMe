...
export class PosController {
    ...
    @Post('cart')
    @Roles(RoleName.RESTAURANT_OWNER, RoleName.CASHIER)
    createCart(
        @Request() req: { user: { restaurantId?: number | null; userId: number } },
        @Body() body?: CreateCartDto,
    ) {
        return this.posService.createCart(req.user.restaurantId, req.user.userId, body?.tableId, body?.terminalLabel);
    }

    @Get('active-sessions')
    @Roles(RoleName.SUPER_ADMIN, RoleName.RESTAURANT_OWNER, RoleName.CASHIER)
    ...
    @Roles(RoleName.RESTAURANT_OWNER, RoleName.CASHIER)
    updateQty(@Param('id') id: number, @Body() dto: UpdateQuantityDto) {
