...
export class PosService {
    ...
    async createCart(
        ...
        cashierUserId: number | null | undefined,
        tableId?: number,
    ...
    async getCart(id: number) {
        return this.cartRepo.findOne({ where: { id }, relations: ['cashier'] });
    }

    async listActiveSessions(restaurantId: number | null | undefined, role: string) {
        if (
            (restaurantId == null || restaurantId === undefined || Number.isNaN(Number(restaurantId))) &&
    ...
    async addItem(cartId: number, dto: AddItemDto) {
        const { menuItemId, name, quantity, price } = dto;
        ...
        const line = { menuItemId, name, quantity, price, modifiers: dto.modifiers };
    ...
    async checkout(cartId: number, dto: CheckoutDto, _userId: number, restaurantId: number) {
        ...
        const pct = cart.discountPercent ?? 0;
